$(document).ready(function () {
  ...
  $('#quiz-form').on('submit', function (event) {
    ...
    const quizFormUrl = 'http://' + window.location.hostname + ':5001/api/mrk/concepts/' + conceptId + '/quizzes';
  ...
  $('#search_input').keydown(function (event) {
    if (event.keyCode === 13) {
      ...
      let searchUrl;
      if (!keyword) {
        searchUrl = 'http://' + window.location.hostname + ':5001/api/mrk/concepts';
      }
      else {
        searchUrl = 'http://' + window.location.hostname + ':5001/api/mrk/concepts_search/' + keyword;
      }
